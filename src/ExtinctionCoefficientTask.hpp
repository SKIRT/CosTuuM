...
public:
   ...
   */
  inline void get_forward_scattering_matrix(const uint_fast32_t itheta_in,
  ...
  virtual void execute(const int_fast32_t thread_id) {
    ...
    for (uint_fast32_t itheta_in = 0; itheta_in < ntheta; ++itheta_in) {
      ...
      get_forward_scattering_matrix(itheta_in, 1., 0., S);
      ...
      _result._Qext[itheta_in] = prefactor * (S[0] + S[3]).imag();
      _result._Qextpol[itheta_in] = prefactor * (S[0] - S[3]).imag();
      _result._Qextcpol[itheta_in] = prefactor * (S[3] - S[0]).real();
