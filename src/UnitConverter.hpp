...
public:
  ...
  static inline Unit get_single_unit(std::string name) {
    ...
    if (name == "m") {
      ...
      /// error handler
      ctm_error("Unknown unit: \"%s\"!", name.c_str());
  ...
  static inline std::string get_SI_unit_name(const int quantity) {
    ...
    case QUANTITY_VOLUME:
      return "m^3";
    default:
      ctm_error("Unknown quantity: %i!", quantity);
      return "";
    }
  }
  ...
  static inline double try_conversion(double value, Unit unit_from,
    ...
    ctm_error("No known conversion from \"%s\" to \"%s\"!",
  ...
  static inline double convert(double value, std::string unit_from,
    ...
    if (!strange_unit_to.is_same_quantity(strange_unit_from)) {
      ...
      ctm_error("Units are not compatible: \"%s\" and \"%s\"!",
