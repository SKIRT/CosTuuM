...
_integer_type_ string_to_integer(const std::string &value) {
  ...
  if (value.size() == 0) {
    ctm_error("Cannot extract an integer from an empty string!");
  }
  ...
  if (idx == value.size()) {
    ctm_error("String does not contain an integer: \"%s\"!", value.c_str());
  }
  ...
  if (value[idx] == '0' && (value[idx + 1] == 'x' || value[idx + 1] == 'X')) {
    ...
    if (digits.size() == 0) {
      ctm_error("Cannot convert string to integer: \"%s\"!", value.c_str());
    }
    if (digits.size() * 4 >
        sizeof(_integer_type_) * 8 - std::is_signed<_integer_type_>::value) {
      ctm_error("Integer value too large: \"%s\"!", value.c_str());
    }
 ...
 */
template <>
inline unsigned long int convert<unsigned long int>(const std::string &value) {
  const unsigned long int ivalue = string_to_integer<unsigned long int>(value);
  return ivalue;
}
...
template <> inline bool convert<bool>(const std::string &value) {
  ...
  if (value_copy == "true" || value_copy == "yes" || value_copy == "on" ||
    ...
    ctm_error("Error converting \"%s\" to a boolean value!",
              value_copy.c_str());
    return false;
  }
}
...
/**
 * @brief Convert the given string to a double precision complex value.
 *
 * @param value std::string value.
 * @return Double precision complex floating point stored in the string.
 */
template <>
inline std::complex<double>
convert<std::complex<double>>(const std::string &value) {

  double real, imag;
  const int_fast32_t num_found =
      sscanf(value.c_str(), "%lf + %lfi", &real, &imag);
  if (num_found != 2) {
    ctm_error("Error converting \"%s\" to a floating point complex value!",
              value.c_str());
  }
...
/**
 * @brief Convert the given complex value to a std::string.
 *
 * @param value Value to convert.
 * @return std::string.
 */
...
inline std::pair<double, std::string> split_value(const std::string &svalue) {
  ...
  try {
    ...
    ctm_error("Error extracting value from \"%s\" unit-value pair!",
              svalue.c_str());
  }
...
inline std::string byte_unit(uint_fast8_t exponent) {
  ...
  case 4:
    return "TB";
  default:
    ctm_error("Name for 2^(10*%u) bytes was not implemented!", exponent);
    return "";
  }
}
