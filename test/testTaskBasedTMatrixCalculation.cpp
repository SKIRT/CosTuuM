...
int main(int argc, char **argv) {
  ...
  for (uint_fast32_t i = 0; i < maximum_order - minimum_order; ++i) {
    quadrature_points[i] = new GaussBasedResources(ndgs * (minimum_order + i));
  ...
  std::vector<InteractionResource *> interactions(maximum_order - minimum_order,
                                                  nullptr);
  for (uint_fast32_t i = 0; i < maximum_order - minimum_order; ++i) {
    interactions[i] =
        new InteractionResource(wavelength, refractive_index, minimum_order + i,
  ...
  std::vector<TMatrixM0Task *> m0tasks(maximum_order - minimum_order, nullptr);
  for (uint_fast32_t i = 0; i < maximum_order - minimum_order; ++i) {
    m0tasks[i] = new TMatrixM0Task(
        tolerance, minimum_order + i, ndgs * (minimum_order + i), nfactors,
        *quadrature_points[i], *geometries[i], *interactions[i], *wignerdm0[i],
        *auxspace[0], Tmatrix, Tmatrix.get_m_resource(0));
