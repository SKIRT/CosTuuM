...
d1 = np.fromfile("Draine_d1.dat").reshape((len(wavelengths), 2))
d05 = np.fromfile("Draine_d05.dat").reshape((len(wavelengths), 2))
d2 = np.fromfile("Draine_d2.dat").reshape((len(wavelengths), 2))
dCDE = np.fromfile("Draine_CDE.dat").reshape((len(wavelengths), 2))

pl.semilogx(wavelengths * 1.0e6, d05[:, 0] / d1[:, 0], "-", label="$d=1/2$")
pl.semilogx(wavelengths * 1.0e6, d2[:, 0] / d1[:, 0], "-", label="$d=2$")
pl.semilogx(
    wavelengths * 1.0e6, dCDE[:, 0] / d1[:, 0], "-", label="$d\\in{}P_{DH}(d)$"
)
pl.xlim(10.0, 1000.0)
pl.xlabel("$\\lambda{}$ ($\\mu{}$m)")
pl.ylabel("$Q_{abs}/Q_{abs,sph}$")
pl.legend(loc="best")
pl.title("$a = {0:.1f}~\\mu{{}}$m".format(sizes[0] * 1.0e6))
pl.tight_layout()
pl.savefig("1993Draine_shapes.png", dpi=300, bbox_inches="tight")
pl.savefig("1993Draine_shapes.pdf", dpi=300, bbox_inches="tight")
