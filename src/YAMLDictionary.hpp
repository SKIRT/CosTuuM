...
private:
  ...
  read_keyvaluepair(std::string &line) {
    ...
    if (colonpos == std::string::npos) {
      ctm_error("Error while parsing line \"%s\": no ':' found!", line.c_str());
    }
...
public:
      ...
      if (!is_comment_line(line) && !is_empty_line(line)) {
        ...
        if (indentation > 0) {
          ...
          if (levels.size() != groupname.size()) {
            ctm_error("Line has a different indentation than expected: \"%s\"!",
                      line.c_str());
          }
   ...
   */
  inline std::map<std::string, std::string>::iterator get_begin_used_values() {
...
inline std::string YAMLDictionary::get_value<std::string>(std::string key) {
  ...
  uint_fast32_t count = _dictionary.count(key);
  if (count == 0) {
    ctm_error("Parameter \"%s\" not found!", key.c_str());
  }
